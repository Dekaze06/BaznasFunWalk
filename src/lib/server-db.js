...
const SEED_PARTICIPANTS = [
  ...
  },
  {
    ...
    nominalDonasi: 0,
    totalBayar: 50000,
    metodeBayar: 'Midtrans (Bank Muamalat Virtual Account)',
    status: 'PAID',
...
const DEFAULT_SETTINGS = {
  ...
  hargaFamilyPack: 180000,
  donasiReguler: 20000,
  donasiFamily: 20000,
  targetPeserta: 2000,
  targetDonasi: 50000000,
  rekeningBank: 'Bank Muamalat Virtual Account via Midtrans Payment Gateway (Otomatis 24 Jam)'
};
...
export function getDashboardStats() {
  ...
  const stockSummary = JERSEY_SIZES.map(item => {
    const used = activeParticipants.filter(p => p.ukuranJersey === item.size).length +
                 activeParticipants.filter(p => p.ukuranJersey2 === item.size).length;
  ...
  return {
    ...
    totalDonasi,
    totalRegistrasi,
    targetDonasi: Number(settings.targetDonasi) || 50000000,
    targetPeserta: Number(settings.targetPeserta) || 2000,
    stockSummary
  };
}
